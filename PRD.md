...
mediagram — социальный трекер потребления медиа ( книги, фильмы, сериалы, аниме, игры ) с авто-фиксацией реального просмотра через browser extension и приватными circles друзей/семьи.

Один продукт вместо пяти аккаунтов в Goodreads / Letterboxd / Trakt / AniList / Backloggd.
...
- **Один клик**: на любом info-сайте из белого списка popup расширения уже распарсил карточку — добавление одним кликом, без поиска и копипасты.
...
**Secondary**: пары и семьи — shared watchlist на пятницу, совместный backlog игр, общий список «что почитать в отпуске».
...
- На `info`-страницах ( карточки kinopoisk, imdb, goodreads, livelib, fantlab, steam store, HLTB, shikimori, MAL, myshows, letterboxd ) **авто-записей не происходит никогда**
- v2: вместо таймера — события `<video>` внутри iframe плеера ( all_frames + host permission домена плеера )

**One-click add ( info-страницы ):**
- Юзер открыл карточку → popup показывает распознанное ( title, year, type, постер ) → кнопка «добавить» → одна запись с выбранным статусом ( дефолт в выпадашке — `want_to` )
- Книги и игры в браузере не потребляются → для них это основной путь вместе с поиском F3
...
- Popup на распознанной info-странице: карточка + «добавить» одним кликом, экстрактор уже распарсил данные и постер
...
- `doing` — смотрю/читаю/играю ( единственный авто-статус )
- `done` — закончил
- `dropped` — бросил
...
5. IGDB / Steam ( опционально )

**Дедуп — механизм:**
- Universal key — `wikidata_id`
- Резолв site_id → QID через Wikidata search `haswbstatement` ( свойства внешних ID: IMDb `P345`, Kinopoisk `P2603`, Steam `P1733`, MAL `P4086` и т.п. )
...
- `bog/mediagram/ext/` — extension ( background + content scripts + popup + bridge-port )
...
export class $bog_mediagram_media extends $giper_baza_entity.with({
	...
	Kind: $giper_baza_atom_enum([ 'movie', 'series', 'book', 'game', 'anime' ]),
...
- **Popup extension** = распознанная карточка текущей страницы + «добавить» одним кликом + статус трекинга сайта
...
- [ ] Extension: popup, content-script таймер, каркас optional_host_permissions
- [ ] **Bridge-port прототип**: `$giper_baza_port` поверх postMessage, whoami/adopt auth, e2e с CSP gh-pages
- [ ] **2 extractor end-to-end**: animego player → `doing` авто; kinopoisk info → one-click из popup
...
| Риск | Решение |
|---|---|
| Сайты меняют DOM → extractor ломается | Snapshot-тесты html + алерт в popup «extractor сломан, обнови» |
| SPA меняет URL без reload → таймер не с той страницы | Хуки pushState / Navigation API, тесты на shikimori/myshows |
| 15-мин порог не добивает на коротких эпизодах | One-click в popup; v2 — события `<video>` в iframe |
...
- **Steam playtime API / Plex / Jellyfin**: auto для игр и домашних медиатек
...
- 6+ source sites: one-click add из popup + постеры
